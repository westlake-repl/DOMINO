...
def main():
    ...
    dataset = Step2Dataset(jsonl_path=args.jsonl_path, tokenizer=tokenizer)
    logger.info(f"数据集大小: {len(dataset)}")

    # 检查断点续传：读取已完成的索引
    completed_indices = set()
    output_path = os.path.join(args.output_dir, os.path.basename(args.jsonl_path).replace(".tsv", ".jsonl"))
